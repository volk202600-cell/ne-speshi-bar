...
function cardFor(it){
  ...
  img.src=it.photo; img.alt=it.name;
  img.onclick=()=>openImg(it.photo);

  const title=document.createElement('div');
  title.className='title'; title.innerText=it.name;

  const desc=document.createElement('div');
  desc.className='desc'; desc.innerText=it.description||'';

  const price=document.createElement('div');
  price.className='price'; price.innerText=fmt(it.price||0);

  const controls=document.createElement('div');
  controls.className='controls';

  const add=document.createElement('button');
  add.className='btn primary add-to-cart';
...
function addToCart(it){
  const f=state.cart.find(x=>x.productId===it.productId);
  if(f) f.qty+=1;
...
function renderCart(){
  ...
  state.cart.forEach(ci=>{
    ...
    row.querySelector('.inc').onclick=()=>changeQty(ci.productId,1);
    row.querySelector('.dec').onclick=()=>changeQty(ci.productId,-1);
  });
  ...
  document.getElementById('cart-total').innerText =
    state.cart.reduce((s,i)=>s + i.qty * (i.price||0),0) + ' ₴';
}

function changeQty(pid,delta){
  state.cart = state.cart
    .map(c=> c.productId===pid? {...c,qty:Math.max(0,c.qty+delta)}:c)
    .filter(c=>c.qty>0);
  saveCart();
  renderCart();
}
...
document.getElementById('checkout').addEventListener('click', async ()=>{
  ...
  const phone = prompt('Телефон (наприклад +380...)');
  if(!phone) return;

  const address = prompt('Адреса доставки');
  if(!address) return;

  const comment = prompt('Коментар до замовлення (необовʼязково)') || '—';

  const payment =
    confirm('Онлайн? OK = Онлайн (неактивно), Cancel = Готівка')
      ? 'online'
      : 'cash';
