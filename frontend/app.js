...
const MENU = {
  categories: [
    { id: 'mains', name: 'Гарячі позиції', items: [
      { id:'i1', name:'Мідії у вершковому соусі з чіабатою', description:'Ніжні мідії у вершковому соусі, подаються з чіабатою.', price:330, img:'images/midii.jpg' },
      { id:'i2', name:'Стейк зі свинини', description:'Соковитий стейк зі свинини — ціна за 100 г.', price:200, img:'images/svynyna.jpg' },
      { id:'i3', name:'Стейк Cowboy', description:'Класичний Cowboy — ціна за 100 г.', price:220, img:'images/cowboy.jpg' },
      { id:'i4', name:'Стейк Ribeye', description:'Ribeye — ніжний мармуровий стейк, ціна за 100 г.', price:280, img:'images/ribeye.jpg' }
    ]}
  ]
};
...
function renderMenu(){
  ...
  state.menu.categories.forEach(cat=>{
    const box = document.createElement('div'); box.className='menu-cat';
    ...
    cat.items.forEach(it=>{
      ...
      const img = document.createElement('img'); img.src = it.img; img.alt = it.name; img.onclick = ()=>openModal(it.img);
      ...
      const btn = document.createElement('button'); btn.className='btn'; btn.innerText='Додати'; btn.onclick = ()=>addToCart(it);
      left.appendChild(title); left.appendChild(desc); left.appendChild(price); left.appendChild(btn);
      item.appendChild(img); item.appendChild(left);
...
function addToCart(it){
  const ex = state.cart.find(c=>c.id===it.id);
  if(ex) ex.qty += 1; else state.cart.push({...it, qty:1});
  renderCart();
}
...
function renderCart(){
  ...
  if(state.cart.length===0){ list.innerHTML='<div class="muted">Кошик порожній</div>'; }
    ...
    state.cart.forEach(ci=>{
      ...
      div.innerHTML = `<div>${ci.name} × ${ci.qty}</div><div><span>${ci.price*ci.qty} ₴</span><button data-id="${ci.id}" class="inc">+</button><button data-id="${ci.id}" class="dec">-</button></div>`;
...
document.getElementById('finalize-btn').addEventListener('click', async ()=>{
  ...
  const payload = { customer:{name,phone,address}, items: state.cart, total: state.cart.reduce((s,i)=>s+i.price*i.qty,0), payment_method: pay };
  try{
...
function openModal(src){ const modal = document.getElementById('modal'); document.getElementById('modal-img').src = src; modal.style.display='flex'; }
function closeModal(e){ if(e && e.target && e.target.id==='modal') return modalHide(); modalHide(); }
function modalHide(){ document.getElementById('modal').style.display='none'; document.getElementById('modal-img').src=''; }
